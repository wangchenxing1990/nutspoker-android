...
public class NimUserInfoCache {
    private final static String TAG = "NimUserInfoCache";
    ...
    public interface IFetchCallback {
        void onFetchFinish(@Nullable List<? extends Object> list);
    }
    /**
     * 构建缓存与清理
     */
    ...
    public void getUserInfoFromRemote(List<String> accounts, final RequestCallback<List<NimUserInfo>> callback) {
        NIMClient.getService(UserService.class).fetchUserInfo(accounts).setCallback(new RequestCallback<List<NimUserInfo>>() {
            ...
            public void onSuccess(List<NimUserInfo> users) {
                LogUtil.i(UIKitLogTag.USER_CACHE, "fetch userInfo completed, add users size =" + users.size());
                // 这里不需要更新缓存，由监听用户资料变更（添加）来更新缓存
                if (callback != null) {
     ...
     * 从云信服务器获取批量用户信息[异步]
     */
    public static void getUserListByNeteaseEx(final List<String> list, int curIndex, @Nullable final IFetchCallback iFetchCallback) {
        LogUtil.i(TAG, "getUserListByNeteaseEx curIndex:" + curIndex);
        if(list == null || list.isEmpty()) {
            if (iFetchCallback != null) {
                iFetchCallback.onFetchFinish(null);
            }
            return;
        }
        ...
        NimUserInfoCache.getInstance().getUserInfoFromRemote(tempList, new RequestCallback<List<NimUserInfo>>() {
            @Override
            public void onSuccess(List<NimUserInfo> nimUserInfos) {
                if (theNewIndex + 1 < list.size()) { //未获取完毕继续获取
                    getUserListByNeteaseEx(list, theNewIndex, iFetchCallback);
                } else {
                    if (iFetchCallback != null) {
                        iFetchCallback.onFetchFinish(null);
                    }
    ...
    public String getUserDisplayName(String account) {
        if (DealerConstant.isDealer(account)) {
            return "客服";
        }
    ...
    private Observer<List<NimUserInfo>> userInfoUpdateObserver = new Observer<List<NimUserInfo>>() {
        ...
        public void onEvent(List<NimUserInfo> users) {
            ...
            LogUtil.d(TAG , "userInfoUpdateObserver");
