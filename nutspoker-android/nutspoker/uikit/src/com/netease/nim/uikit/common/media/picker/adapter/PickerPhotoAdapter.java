...
public class PickerPhotoAdapter extends BaseAdapter {
	...
	public void refreshView(int index) {
		...
		if (list.get(index).getChooseIndex() > 0) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		viewHolder.selectHotPot.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (photo.getChooseIndex() > 0) {
					photo.setChooseIndex(0);
					hasSelect--;
				} else if (hasSelect < maxSelectSize) {
					photo.setChooseIndex(1);
					hasSelect++;
				} else {
		...
		if (list.get(position).getChooseIndex() > 0) {
		...
		if (photoInfo != null) {
			...
			PickerlImageLoadTool.disPlay(thumbPath, new RotateImageViewAware(viewHolder.image, photoInfo.getAbsolutePath()), R.drawable.nim_image_default);
		}
