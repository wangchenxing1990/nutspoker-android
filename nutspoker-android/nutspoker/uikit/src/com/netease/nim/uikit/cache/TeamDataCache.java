...
public class TeamDataCache {
    private final static String TAG = "TeamDataCache";
    ...
    public void buildCache() {
        ...
        //圈子成员缓存
        for(Team team : teams){
//            if(team.getType() == TeamTypeEnum.Normal && (TextUtils.isEmpty(team.getName()) || team.getName().equals(ClubConstant.GROUP_IOS_DEFAULT_NAME))){
            if(team.getType() == TeamTypeEnum.Normal){
                LogUtil.d(TAG , "创建圈子成员缓存,teamId:" + team.getId());
                fetchTeamMemberList(team.getId() , null);
            }
        }
    }
     ...
     * 异步获取Team（去服务器查询）
     */
    public void searchTeamById(final String teamId , final SimpleCallback<Team> callback){
        NIMClient.getService(TeamService.class).searchTeam(teamId).setCallback(new RequestCallbackWrapper<Team>() {
            @Override
            public void onResult(int code, Team team, Throwable exception) {
                ...
                if (callback != null) {
                    callback.onResult(success, team);
                }
            }
        });
    }
    ...
    public String getTeamName(String teamId) {
        ...
        if(team != null && team.getType() == TeamTypeEnum.Normal){
            return TextUtils.isEmpty(team.getName()) || ClubConstant.GROUP_IOS_DEFAULT_NAME.equals(team.getName()) ? TeamHelper.getTeamNameByMember(teamId) : team.getName();
        }
    ...
    public void fetchTeamMemberList(final String teamId, final SimpleCallback<List<TeamMember>> callback) {
        LogUtil.d(TAG , "获取圈子成员,teamId:" + teamId);
        NIMClient.getService(TeamService.class).queryMemberList(teamId).setCallback(new RequestCallbackWrapper<List<TeamMember>>() {
            ...
            public void onResult(int code, final List<TeamMember> members, Throwable exception) {
                boolean success = true;
                LogUtil.d(TAG , "获取圈子成员,teamId:" + teamId + "; success :" + success);
                if (code == ResponseCode.RES_SUCCESS) {
