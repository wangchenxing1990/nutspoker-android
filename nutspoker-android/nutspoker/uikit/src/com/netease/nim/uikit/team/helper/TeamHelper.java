...
public class TeamHelper {
     ...
     * @return
     */
    public static String getTeamNameByMember(String teamId) {
        StringBuffer paramsBuf = new StringBuffer();
        String groupName = "";
        List<TeamMember> memberList = TeamDataCache.getInstance().getTeamMemberList(teamId);
        ...
        if (memberList != null && memberList.size() != 0) {
            ...
            int memberNumExcMe = 0;
            for (int i = 0; i < size; i++) {
                TeamMember member = memberList.get(i);
                LogUtil.d(TAG, "position :" + i + ":" + TeamDataCache.getInstance().getTeamMemberDisplayName(teamId, member.getAccount()));
                if(member.getAccount().equals(NimUIKit.getAccount())){
                    //如果是自己，不加入名称
                    continue;
                }
                groupName = groupName + TeamDataCache.getInstance().getTeamMemberDisplayName(teamId, member.getAccount()) + "、";
                if (i == (size - 1) || memberNumExcMe > 4) {
                    break;
                }
                memberNumExcMe = memberNumExcMe + 1;
            }
            if(!TextUtils.isEmpty(groupName)){
                groupName = groupName.substring(0 , groupName.length() -1);
            }
            LogUtil.d(TAG, "size :" + size + ";" + groupName);
        } else {
            LogUtil.d(TAG, "memberList == null");
        }
        if (TextUtils.isEmpty(groupName)) {
            groupName = NimUIKit.getContext().getString(R.string.group);
        }
        return groupName;
    }
}
