...
public class NimSingleThreadExecutor {
    ...
    private WeakReference<Handler> uiHander;
    private Executor executor;

    private NimSingleThreadExecutor() {
        uiHander = new WeakReference<>(NimUIKit.sHandler); //new Handler(NimUIKit.getContext().getMainLooper());
    ...
    private class NimRunnable<T> implements Runnable {
        ...
        public void run() {
            ...
            if (uiHander!= null && uiHander.get() != null) {
                uiHander.get().post(new Runnable() {
                    @Override
                    public void run() {
