...
public class LoginSyncDataStatusObserver {
    ...
    public boolean observeSyncDataCompletedEvent(Observer<Void> observer) {
        ...
        // 超时定时器
        if (uiHandler == null) {
            uiHandler = new WeakReference<>(NimUIKit.sHandler);//new Handler(NimUIKit.getContext().getMainLooper());
        }
        ...
        uiHandler.get().removeCallbacks(timeoutRunnable);
        uiHandler.get().postDelayed(timeoutRunnable, TIME_OUT_SECONDS * 1000);
