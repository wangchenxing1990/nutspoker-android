    ...
    TransactionAdapter(Context context, TransactionListFragment.OnListFragmentInteractionListener listener) {
        ...
        cursorAdapter = new CursorAdapter(TransactionAdapter.this.context, null, CursorAdapter.FLAG_REGISTER_CONTENT_OBSERVER) {
            ...
            public void bindView(View view, final Context context, Cursor cursor) {
                ...
                holder.path.setText(transaction.method + " " + transaction.path);
                holder.host.setText(transaction.host);
                ...
                if (transaction.getStatus() == HttpTransaction.Status.Complete) {
                    holder.code.setText(String.valueOf(transaction.responseCode));
            ...
            private void setStatusColor(ViewHolder holder, HttpTransaction transaction) {
                ...
                } else if (transaction.responseCode >= 500) {
                    color = color500;
                } else if (transaction.responseCode >= 400) {
                    color = color400;
                } else if (transaction.responseCode >= 300) {
                    color = color300;
                } else {
                    color = colorDefault;
                }
