...
import com.htgames.nutspoker.debug.network_monitor.ChuckInterceptor;
...
public class RetentionManager {
    ...
    public RetentionManager(Context context, ChuckInterceptor.Period retentionPeriod) {
        ...
        cleanupFrequency = (retentionPeriod == ChuckInterceptor.Period.ONE_HOUR) ? TimeUnit.MINUTES.toMillis(30) : TimeUnit.HOURS.toMillis(2);
    }
    ...
    public synchronized void doMaintenance() {
        if (period > 0) {
            ...
            if (isCleanupDue(now)) {
                LogUtil.i(LOG_TAG, "Performing data retention maintenance...");
    ...
    private void deleteSince(long threshold) {
        int rows = context.getContentResolver().delete(ChuckContentProvider.TRANSACTION_URI, "requestDate <= ?", new String[] { String.valueOf(threshold) });
        LogUtil.i(LOG_TAG, rows + " transactions deleted");
    }
