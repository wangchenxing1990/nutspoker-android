package com.htgames.nutspoker.view.switchbutton;
		...
		switch (action) {
			...
			case MotionEvent.ACTION_UP:
				...
				if (deltaX < mTouchSlop && deltaY < mTouchSlop && time < mClickTimeout) {
					performClick();
					LogUtil.i("performClick switchbutton  nextStatus: " + nextStatus + "   isChecked(): " + isChecked());
				} else {
					LogUtil.i("noclick switchbutton  nextStatus: " + nextStatus + "   isChecked(): " + isChecked());
					if (nextStatus != isChecked()) {
//						playSoundEffect(SoundEffectConstants.CLICK);
//						setChecked(nextStatus);
						performClick();//这个函数会改变checked的值，所以把上面的一行代码注释   也会有声音的
					} else {
