...
public class CircleMovementMethod extends BaseMovementMethod {
    ...
    public final static int DEFAULT_COLOR = android.R.color.transparent;
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer,
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (mClickLinks.length > 0) {
                ...
                //设置点击区域的背景色
                mBgSpan = new BackgroundColorSpan(DemoCache.getContext().getResources().getColor(mClickableSpanBgClorId));
                int startIndex = buffer.getSpanStart(mClickLinks[0]) <= 0 ? 0 : buffer.getSpanStart(mClickLinks[0]);
                int endIndex = buffer.getSpanEnd(mClickLinks[0]) <= 0 ? 0 : buffer.getSpanEnd(mClickLinks[0]);
                buffer.setSpan(mBgSpan, startIndex, endIndex, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            } else {
                setPassToTv(true);
                // textview选中效果
