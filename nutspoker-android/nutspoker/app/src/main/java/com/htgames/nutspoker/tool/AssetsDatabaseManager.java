 ...
 * It will copy the database file to "/data/data/[your application package nameId]/database" when you first time you use it
...
public class AssetsDatabaseManager {
    ...
    public SQLiteDatabase getDatabase(String dbfile) {
        if (databases.get(dbfile) != null) {
            LogUtil.i(tag, String.format("Return a database copy of %s", dbfile));
            return (SQLiteDatabase) databases.get(dbfile);
        }
        if (context == null)
            return null;

        LogUtil.i(tag, String.format("Create database %s", dbfile));
        ...
        if (!flag || !file.exists()) {
            ...
            if (!file.exists() && !file.mkdirs()) {
                LogUtil.i(tag, "Create \"" + spath + "\" fail!");
                return null;
            }
            if (!copyAssetsToFilesystem(dbfile, sfile)) {
                LogUtil.i(tag, String.format("Copy %s to %s fail!", dbfile, sfile));
                return null;
            }
    ...
    static public void closeAllDatabase() {
        LogUtil.i(tag, "closeAllDatabase");
        if (mInstance != null) {
