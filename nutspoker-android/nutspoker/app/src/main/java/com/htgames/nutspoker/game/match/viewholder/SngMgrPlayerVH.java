...
/**
 * Created by 周智慧 on 17/3/14.
 */

public class SngMgrPlayerVH extends RecyclerView.ViewHolder {
    ...
    public static SngMgrPlayerVH createViewHolder(Context context) {
        return new SngMgrPlayerVH(LayoutInflater.from(context).inflate(R.layout.view_swipe_content, null), context);
    }

    public SngMgrPlayerVH(View itemView, Context context) {
        super(itemView);
...
//        WealthHelper.SetMoneyText(tv_club_myself, bean.reward, itemView.getContext());//sng记分牌直接显示灰色的数字
        tv_club_myself.setText("" + bean.reward);
        mRankView.setRankTagView(bean.rank);
        if (gameStatus == GameStatus.GAME_STATUS_WAIT) {
            mRankView.setVisibility(View.GONE);
        }
        mtt_hunter_container.setVisibility(View.GONE);
